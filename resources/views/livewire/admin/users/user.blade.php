<div>
    <x-ag.header-admin current-text="Benutzer verwalten" render="benutzerVerwalten">
        ...
        @can(['show', 'create', 'update'])
                    ...
                    @elseif($createMode)
                        @can('create')
                            @include('livewire.admin.users.partials.create')
                        @endcan
                    @elseif($showMode)
                        @can('show')
                            @include('livewire.admin.users.partials.show')
                        @endcan
                    @else
                        @can('show')
                                    ...
                                    @forelse($users as $user)
                                        @if($loop->iteration % 2 == 0)
                                            <tr class="border-b bg-gray-50 dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                                <th scope="row" class="px-6 py-1 cursor-pointer" wire:click="show({{ $user->id }})">{{ $user->id }}</th>
                                                <td class="px-6 py-1 cursor-pointer" wire:click="show({{ $user->id }})">{{ $user->name }}</td>
                                                <td class="px-6 py-1 cursor-pointer" wire:click="show({{ $user->id }})">{{ $user->email }}</td>
                                                <td class="px-6 py-1 cursor-pointer" wire:click="show({{ $user->id }})">
                                                    @foreach($user->roles as $role)
                                                        <div class="inline-flex mb-2 xl:mb-0"><x-ag.badge color="blue" class="" text="{{ $role->name }}"/></div>
                                                    @endforeach
                                                </td>
                                                <td class="px-6 py-1 text-right">
                                                    <x-ag.buttonLink wire:click="edit({{ $user->id }})" class="px-3 py-2 text-blue-500 hover:text-blue-600">
                                                    ...
                                                    @if($confirming === $user->id)
                                                        <x-ag.button-link-group>
                                                            <x-ag.buttonLink wire:loading.remove wire:click="delete({{ $user->id }})" class="px-3 py-2 text-red-500 hover:text-red-600" title="{{ __('Delete') }}">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                                    ...
                                                    @elseif($confirming !== $user->id)
                                                        <x-ag.buttonLink wire:click="confirmDelete({{ $user->id }})" class="px-3 py-2 text-red-500 hover:text-red-600">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                        ...
                                        @else
                                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                                ...
                                                <td class="px-6 py-1 cursor-pointer" wire:click="show({{ $user->id }})">
                                                    @foreach($user->roles as $role)
                                                        <div class="inline-flex justify-center mb-2 xl:mb-0"><x-ag.badge color="blue" class="" text="{{ $role->name }}"/></div>
                                                    @endforeach
                                                </td>
                                                <td class="px-6 py-1 text-right">
                                                    <x-ag.buttonLink wire:click="edit({{ $user->id }})" class="px-3 py-2 text-blue-500 hover:text-blue-600">
