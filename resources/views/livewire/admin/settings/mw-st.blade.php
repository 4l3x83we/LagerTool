<div>
    ...
    <x-ag.header-admin current-page="Settings" current-text="Settings VAT" current-route="admin.settings.steuern" :pages="$pages">
        <x-slot:headline></x-slot:headline>
        @can(['create', 'update'])

            @if($updateMode)
                @can('update')
                    @include('livewire.admin.settings.mwst.edit')
                @endcan
            @else
                @can('create')
                    @include('livewire.admin.settings.mwst.create')
                @endcan
            @endif
            ...
            <x-ag.table>
                ...
                <x-slot:tbody>
                    @foreach($mwsts as $mwst)
                        <x-ag.tr>
                            <td class="px-6 py-2">{{ $mwst->mw_name }}</td>
                            <td class="px-6 py-2">{{ $mwst->mw_wert . '%' }}</td>
                            <td class="px-6 py-2">
                                <div class="flex justify-end items-center">
                                    <x-ag.buttonLink wire:click="edit({{$mwst->id}})" class="text-blue-500 hover:text-blue-700 mr-8" icon="fa-pen" />
                                    <x-ag.buttonLink wire:click="destroy({{$mwst->id}})" class="text-red-500 hover:text-red-700 mr-2" icon="fa-trash" />
                                </div>
                            </td>
                        </x-ag.tr>
                    @endforeach
