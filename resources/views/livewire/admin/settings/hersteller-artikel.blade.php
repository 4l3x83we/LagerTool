<div>
    ...
    <x-ag.header-admin current-page="Settings" current-text="Settings Manufacturer Item" current-route="admin.settings.hersteller-artikel" :pages="$pages">
        <x-slot:headline></x-slot:headline>
    @can(['create', 'update'])

        @if($updateMode)
            @can('update')
                @include('livewire.admin.settings.herstellerArtikel.edit')
            @endcan
            <x-ag.hr class="my-4" />
        @else
            @can('create')
                @include('livewire.admin.settings.herstellerArtikel.create')
            @endcan
            <x-ag.hr class="my-4" />
        @endif
            ...
            <x-ag.table>
                ...
                <x-slot:tbody>
                    @foreach($herstellers as $hersteller)
                        <x-ag.tr>
                            <td class="px-6 py-2">{{ $hersteller->ha_name }}</td>
                            <td class="px-6 py-2">
