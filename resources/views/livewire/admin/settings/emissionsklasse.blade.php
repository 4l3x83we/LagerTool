            ...
            <x-ag.table>
                <x-slot:thead>
                    <tr>
                        ...
                        <th class="px-6 py-2">Kat</th>
                        <th class="px-6 py-2"></th>
                    </tr>
                </x-slot:thead>
                <x-slot:tbody>
                    @foreach($emissionsklassen as $emissionsklasse)
                        <x-ag.tr>
                            <td class="px-6 py-2">{{ $emissionsklasse->emissionsklasse }}</td>
                            <td class="px-6 py-2">{!! Emissionsklasse::katName($emissionsklasse->kat_id) !!}</td>
                            <td class="px-6 py-2">
                                <div class="flex justify-end items-center">
                                    <x-ag.buttonLink wire:click="destroy({{$emissionsklasse->id}})" class="text-red-500 hover:text-red-700 mr-2" icon="fa-trash"/>
                                </div>
                            </td>
                        </x-ag.tr>
                    @endforeach
