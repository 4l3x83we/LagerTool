@php use App\Models\Admin\HSN; @endphp
<div>
    ...
    <x-ag.header-admin current-text="Settings Manufacturer" render="hsn">
        <x-slot:headline></x-slot:headline>
        @can(['create', 'update'])

            @if($updateMode)
                @can('update')
                    @include('livewire.admin.settings.hsn.edit')
                @endcan
                <x-ag.hr class="my-4"/>
            @else
                @can('create')
                    @include('livewire.admin.settings.hsn.create')
                @endcan
                <x-ag.hr class="my-4"/>
            @endif
            ...
            <div class="grid grid-cols-1 gap-y-4 mb-4">
                <x-ag.table>
                    ...
                    <x-slot:tbody>
                        @foreach($hsns as $hsn)
                            <x-ag.tr>
                                <td class="px-6 py-2">{{ $hsn->hsn }}</td>
                                <td class="px-6 py-2">{{ $hsn->hersteller->hr_name }}</td>
                                <td class="px-6 py-2">
                                    <div class="flex justify-end items-center">
                                        <x-ag.buttonLink wire:click="edit({{$hsn->id}})" class="text-blue-500 hover:text-blue-700 mr-8" icon="fa-pen"/>
                                        <x-ag.buttonLink wire:click="destroy({{$hsn->id}})" class="text-red-500 hover:text-red-700 mr-2" icon="fa-trash"/>
                                    </div>
                                </td>
                            </x-ag.tr>
                        @endforeach
