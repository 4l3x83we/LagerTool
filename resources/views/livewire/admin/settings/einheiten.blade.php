<div>
    ...
    <x-ag.header-admin current-page="Settings" current-text="Settings Units" current-route="admin.settings.einheiten" :pages="$pages">
        ...
        @if($updateMode)
            @can('update')
                @include('livewire.admin.settings.einheiten.edit')
            @endcan
        @else
            @can('create')
                @include('livewire.admin.settings.einheiten.create')
            @endcan
        @endif
        ...
        <div class="grid grid-cols-1 gap-y-4 mb-4">
            <x-ag.table>
                ...
                <x-slot:tbody>
                    @foreach($einheitens as $einheiten)
                        <x-ag.tr>
                            <td class="px-6 py-2">{{ $einheiten->eh_name }}</td>
                            <td class="px-6 py-2">
                                <div class="flex justify-end items-center">
                                    <x-ag.buttonLink wire:click="edit({{$einheiten->id}})" class="text-blue-500 hover:text-blue-700 mr-8" icon="fa-pen" />
                                    <x-ag.buttonLink wire:click="destroy({{$einheiten->id}})" class="text-red-500 hover:text-red-700 mr-2" icon="fa-trash" />
                                </div>
                            </td>
                        </x-ag.tr>
                    @endforeach
