    ...
    <div class="px-6 mx-auto grid">
        <div class="w-full overflow-hidden">
            <div class="w-full overflow-x-auto">
                ...
                <x-ag.breadcrumb home="Dashboard" page="{{ __('Permissions') }}" route="admin.permission" text="{{ __('Permissions') }}">
                    @if(!empty($pages))
                ...
                <div class="flex items-center justify-between flex-wrap my-4">
                    @can(['show', 'create', 'update'])
                        @if($updateMode)
                            @can('update')
                                <span class="text-lg text-gray-500 md:text-xl dark:text-gray-400">{{ $permission->name }}</span>
                        ...
                        @elseif($createMode)
                            @can('create')
                                <span class="text-lg text-gray-500 md:text-xl dark:text-gray-400">{{ __('Add new permission') }}</span>
                                <x-ag.button wire:click="updateModeClose()" class="px-3 py-2 bg-red-500 hover:bg-red-600 dark:bg-red-500 dark:hover:bg-red-600 text-xs" icon-left="fa-rotate-left" button-text="{{ __('Back') }}"/>
                            @endcan
                        @else
                            <span class="text-lg text-gray-500 md:text-xl dark:text-gray-400">{{ __('Manage your permissions here.') }}</span>
                            @can('show')
                                <x-ag.button wire:click="create()" class="px-3 py-2 text-xs" icon-left="fa-plus" button-text="{{ __('Add New Permission') }}"/>
                            @endcan
                        @endif
                    @endcan
                </div>
                ...
                @can(['show', 'create', 'update'])
                    @if($updateMode)
                        @can('update')
                            @include('livewire.admin.permission.partials.edit')
                        @endcan
                    @elseif($createMode)
                        @can('create')
                            @include('livewire.admin.permission.partials.create')
                        @endcan
                    @else
                        @can('show')
                                ...
                                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                    ...
                                    @forelse($permissions as $permission)
                                        @if($loop->iteration % 2 == 0)
                                            <tr class="border-b bg-gray-50 dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                                <td class="px-6 py-1">{{ $permission->name }}</td>
                                                <td class="px-6 py-1">{{ $permission->guard_name }}</td>
                                                <td class="px-6 py-1 text-right">
                                                    <x-ag.buttonLink wire:click="edit({{ $permission->id }})" class="px-3 py-2 text-blue-500 hover:text-blue-600">
                                                    ...
                                                    @if($confirming === $permission->id)
                                                        <x-ag.button-link-group>
                                                            <x-ag.buttonLink wire:loading.remove wire:click="delete({{ $permission->id }})" class="px-3 py-2 text-red-500 hover:text-red-600" title="{{ __('Delete') }}">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                                    ...
                                                    @else
                                                        <x-ag.buttonLink wire:click="confirmDelete({{ $permission->id }})" class="px-3 py-2 text-red-500 hover:text-red-600">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                    ...
                                    @empty
                                        <tr class="odd:bg-white even:bg-gray-50 dark:odd:bg-gray-900 dark:even:bg-gray-800">
                                            <td colspan="5">{{ __('No permissions found.') }}</td>
                                        </tr>
                                    @endforelse
                                    </tbody>
                                </table>
