<div>
    <x-ag.header-admin current-text="" render="fahrzeugeCreate">
       {{-- <x-slot:headline>
        </x-slot:headline>--}}

        <div class="flex items-center justify-between pb-4">
            <span class="text-lg text-gray-500 md:text-xl dark:text-gray-400">{{ __('Add New Vehicle') }}</span>
            <div>
                ...
                <form wire:submit.prevent="store">
                    ...
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-2">
                        ...
                        <div class="grid grid-cols-1 gap-4 place-content-start">
                            @if($formStep == 1)
                                <x-ag.form.form-inline id="fz_id" text="int. Fz.-Nr." stern="true" class-label="sm:basis-1/4" class-input="sm:basis-3/4" placeholder="Interne Fahrzeugnummer wird automatisch generiert." disabled/>
                                <div>
                                    ...
                                    <div class="grid grid-cols-1 xl:gap-2 xl:grid-cols-2">
                                        <x-ag.form.form-inline id="fahrzeug.fz_hsn" maxlength="4" value="null" text="HSN (zu 2.1)" stern="true" class-label="xl:basis-1/2 sm:basis-1/4" class-input="xl:basis-1/2 sm:basis-3/4" />
                                        <x-ag.form.form-inline id="fahrzeug.fz_tsn" maxlength="9" text="TSN (zu 2.2)" stern="true" class-label="xl:basis-1/2 sm:basis-1/4" class-input="xl:basis-1/2 sm:basis-3/4" />
                                    </div>
                                    <x-ag.form.form-select-inline id="fahrzeug.hersteller_id" text="Hersteller (zu D.1)" class-select="sm:basis-3/4" class-label="sm:basis-1/4" >
                                        <option value="">Bitte wähle einen Hersteller aus oder gib eine HSN ein</option>
                                        @foreach($herstellers as $hersteller)
                                            <option value="{{ $hersteller->id }}" {{ old('fahrzeug.fz_hersteller') == $hersteller->hr_name ? 'selected' : '' }}>{{ $hersteller->hr_name }}</option>
                                        @endforeach
                                    </x-ag.form.form-select-inline>
                                    <x-ag.form.form-select-inline id="fahrzeug.model_id" text="Modell (zu D.3)" class-select="sm:basis-3/4" class-label="sm:basis-1/4" >
                                        <option value="">Bitte wähle ein Model aus oder gib eine TSN ein</option>
                                        @foreach($models as $model)
                                            <option value="{{ $model->id }}" {{ old('fahrzeug.fz_model') == $model->md_name ? 'selected' : '' }}>{{ $model->md_name }}</option>
                                        @endforeach
                                    </x-ag.form.form-select-inline>
                                    <x-ag.form.form-inline id="fahrzeug.fz_type" text="Type (zu D.2)" stern="true" class-label="sm:basis-1/4" class-input="sm:basis-3/4" />
                                    <x-ag.form.form-inline id="fahrzeug.fz_fz_id_nr" text="Fz.-Ident.-Nr. (zu E)" stern="true" class-label="sm:basis-1/4" class-input="sm:basis-3/4" count="true" limit="17" />
                                ...
                                <div>
                                    ...
                                    <div class="flex flex-row items-center justify-start">
                                        ...
                                        <x-ag.radio id="kunden_neu_1" wire:model="kunde_neu" text="Neukunde" value="0" />
                                        <x-ag.radio id="kunden_neu_2" wire:model="kunde_neu" text="Bestandskunde" value="1" />
                                    </div>
                                    @if(!$kunde_neu)
                                        <div class="grid grid-cols-1 xl:gap-2 xl:grid-cols-2">
                                            <x-ag.form.form-inline id="kunde.kd_vorname" text="Vorname" stern="true" class-label="xl:basis-1/2 sm:basis-1/4" class-input="xl:basis-1/2 sm:basis-3/4" />
                                            <x-ag.form.form-inline id="kunde.kd_name" text="Name/Firma" stern="true" class-label="xl:basis-1/2 sm:basis-1/4" class-input="xl:basis-1/2 sm:basis-3/4" />
                                        </div>
                                        <x-ag.form.form-inline id="kunde.kd_strasse" text="Straße" stern="true" class-label="sm:basis-1/4" class-input="sm:basis-3/4" />
                                        <div class="grid grid-cols-1 xl:gap-2 xl:grid-cols-2">
                                            <x-ag.form.form-inline id="kunde.kd_plz" maxlength="5" text="PLZ" stern="true" class-label="xl:basis-1/2 sm:basis-1/4" class-input="xl:basis-1/2 sm:basis-3/4" />
                                            <x-ag.form.form-inline id="kunde.kd_ort" text="Ort" stern="true" class-label="xl:basis-1/2 sm:basis-1/4" class-input="xl:basis-1/2 sm:basis-3/4" />
                                        </div>
                                    @else
                                        <x-ag.form.form-select-inline id="kunden_selected" text="Kunde auswählen" class-select="sm:basis-3/4" class-label="sm:basis-1/4" >
                                                <option value="">Bitte wähle einen Kunden aus</option>
                                            @foreach($kundens as $kunde)
                                                @if($kunde->id == '1')
                                                    <option value="{{ $kunde->id }}" {{ old('kunden_selected') == $kunde->id ? 'selected' : '' }}>{{ $kunde->kd_kundennummer . ', ' . $kunde->fullname() }}</option>
                                                @else
                                                    <option value="{{ $kunde->id }}" {{ old('kunden_selected') == $kunde->id ? 'selected' : '' }}>{{ $kunde->kd_kundennummer . ', ' . $kunde->fullname() . ', ' . $kunde->kd_strasse . ', ' . $kunde->kd_plz . ', ' . $kunde->kd_ort }}</option>
                                                @endif
                                            @endforeach
                                        </x-ag.form.form-select-inline>
                                        @if($kunden_selected)
                                            ...
                                            <div class="grid grid-cols-1 xl:gap-2 xl:grid-cols-2">
                                                <x-ag.form.form-inline id="kunde.kd_plz" maxlength="5" text="PLZ" stern="true" class-label="xl:basis-1/2 sm:basis-1/4" class-input="xl:basis-1/2 sm:basis-3/4" disabled/>
                            ...
                            @if($formStep == 2)
                                <div>
                                    ...
                                    <x-ag.form.form-select-inline id="fahrzeug.fz_farbe" text="Farbe" class-label="sm:basis-1/4" class-select="sm:basis-2/4" >
                                        <option value="">Bitte wähle eine Farbe aus</option>
                        ...
                        <div class="grid grid-cols-1 gap-4 mb-4 place-content-start">
                            @if($formStep == 1)
                                <div>
                                    ...
                                    <x-ag.form.form-inline type="month" id="fahrzeug.fz_hu" text="HU" stern="true" class-label="sm:basis-1/4" class-input="sm:basis-3/4" />
                                    ...
                                    <x-ag.form.form-select-inline id="fahrzeug.fz_treibstoff" text="Kraftstoff" class-select="sm:basis-3/4" class-label="sm:basis-1/4" >
                                        <option value="">Bitte Kraftstoff auswählen</option>
                                    ...
                                    <div class="grid grid-cols-1 xl:gap-2 xl:grid-cols-2">
                                        <x-ag.form.form-select-inline id="fahrzeug.fz_kat" text="KAT" class-label="xl:basis-1/2 sm:basis-1/4" class-select="xl:basis-1/2 sm:basis-3/4" >
                                            <option value="">Bitte Euro Norm auswählen</option>
                                        ...
                                        <x-ag.form.form-select-inline id="fahrzeug.fz_plakette" text="Plakette" class-label="xl:basis-1/2 sm:basis-1/4" class-select="xl:basis-1/2 sm:basis-3/4" >
                                            <option value="">Bitte Umweltplakette auswählen</option>
                                    ...
                                    <x-ag.form.form-select-inline id="fahrzeug.fz_emissionsklasse" text="Emissionsklasse" class-select="sm:basis-3/4" class-label="sm:basis-1/4" >
                                        <option value="">Bitte Emissionsklasse auswählen</option>
                                ...
                                <div>
                                    ...
                                    <x-ag.textarea id="fahrzeug.fz_infos" text="" rows="14"><x-slot:content>{{ old('fahrzeug.fz_infos') }}</x-slot:content></x-ag.textarea>
                                </div>
                            @endif
                            @if($formStep == 2)
                                <div>
                        ...
                        <div class="grid grid-cols-1 xl:col-span-2 gap-4 place-content-start">
                            <div class="flex justify-between items-center" wire:loading.remove>
                                ...
                                @if($formStep == 1)
                                    <div class="text-right">
                                        ...
                                        <x-ag.buttonLink type="submit" icon-left="fa-save" text="Save" wire:ignore/>
                                    </div>
                                @endif
                                @if($formStep == 2)
                                    <div class="text-right">
                            ...
                            <div wire:loading wire:target="store" class="flex justify-center items-center">
